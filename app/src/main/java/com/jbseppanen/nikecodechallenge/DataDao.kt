...
object DataDao {
    ...
    interface DataCallback {
        fun callback(success: Boolean, albums: ArrayList<Album>)
    }
                ...
                override fun returnResult(success: Boolean, result: String) {
                    ...
                    if (success) {
                        ...
                        for (i in 0 until jsonArray.length()) {
                            try { //Album would be skipped if error parsing Json.  Could run a try/catch on each as was done with contentAdvisoryRating.
                    ...
                    callback.callback(success, albums)
                }
            })
    }
}